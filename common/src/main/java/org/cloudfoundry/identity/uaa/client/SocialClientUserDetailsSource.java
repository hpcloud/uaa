...
/**
 * <p>
 * Helper class to extract user details from a remote endpoint, triggering authentication and approval if necessary by
 * throwing an appropriate exception. The exception and how it should be handled depends on the authorization protocol
 * in use (a RestTemplate implementation is usually paired with a client-side filter that handles redirects to the
 * provider).
 * </p>
 * 
 * <p>
 * Tested with Facebook, Github, Cloudfoundry, Twitter, LinkedIn. Should work with other providers if they use the same
 * conventions in their user info endpoint. Note that the amount of information contained in the resulting
 * {@link SocialClientUserDetails} depends on the provider and also on the user's preferences with the provider (e.g. he
 * may have chosen not to reveal his email in github, in which case the user details will not have an email address).
 * </p>
 * 
 * @author Dave Syer
 * 
